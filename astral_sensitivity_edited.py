...
file_list_7 = []
file_list_8 = []
listoflists = [file_list_1,file_list_2,file_list_3,file_list_4,file_list_5,file_list_6,file_list_7,file_list_8]
eucl_file = open("bins23456_euclidean_dist.csv", 'r')
count = 0
...
for lists in listoflists:
        x += 1 
        file_1 = "bins23456_inclusion"+str(x)+"_genetrees.tre"
        file_2 = "bins23456_inclusion"+str(x)+"_bootstrap.txt"
        inclusion = open(file_1, 'w')
        bootstrap = open(file_2, 'w')
	for loci in lists:
		loci_file = open("bins23456_RAxML/RAxML_bestTree."+loci+"_Bins23456_concatenated.phy.out", 'r')
		for line in loci_file:
			print >>inclusion, line
		bs_file = "bins23456_RAxML/RAxML_bootstrap."+loci+"Bins23456_concatenated.phy.out"
		print >>bootstrap, bs_file
	inclusion.close()
	bootstrap.close()
	#run astral process
	astral_out = "bins23456_inclusion"+str(x)+"_astral.tre"
	...
	boot_out = "bins23456_inclusion"+str(x)+"_astral_bs.tre"
	boot_call = "java -jar "+astral_path+" --input "+astral_out+" -b "+file_2+" --output "+boot_out
